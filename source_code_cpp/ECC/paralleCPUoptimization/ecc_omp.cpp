...
int get_random() {
    ...
    static std::uniform_int_distribution<> dis(1, 1000000000000000000);
    return dis(gen);
}
...
#pragma omp parallel for num_threads(2)
    for (int i = 0; i < 2; i++) {
        if (i == 0) {
            ...
            auto end = std::chrono::high_resolution_clock::now();
            std::cout << "Read file time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        }
        else {
            auto start = std::chrono::high_resolution_clock::now();
            ...
            auto end = std::chrono::high_resolution_clock::now();
            std::cout << "Rand k time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        }
    }
...
#pragma omp parallel for 
    for (int i = 0; i < data.size(); i++) {
        ...
        ecc_encrypt(points[i].first, points[i].second, plaintext, G, public_key, ks[i]);
        ns[i] = n;
    }
    auto end = std::chrono::high_resolution_clock::now();
